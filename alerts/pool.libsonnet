        ...
        rules: [
          {
            ...
            expr: |||
              ceph_pool_stored{%(cephExporterSelector)s} / (ceph_pool_stored{%(cephExporterSelector)s} + ceph_pool_max_avail{%(cephExporterSelector)s})
              * on(pool_id) group_right ceph_pool_metadata{%(PoolNearFullFilter)s} > %(PoolNearFullThreshold)s
            ||| % $._config,
